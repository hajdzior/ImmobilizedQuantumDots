/*-------------------------------------
  Last mod: 24.09.14
#		Changes: 
			was: counts --> left border of the bin
			is: counts --> right border of the bin (like in SymPhoTime software)
  The programme reads the bin file,
  from the interarrival times calculates the trajectory wyth the given bin size.
  The bin size (in seconds) is given as the parameter during executing
...
************************************************************************/
// compile me: 
// g++ ReadIATandCalcTrajs.c -lcurses
...
void GotPhoton(long long TimeTag, int Channel, int DTime)
{
  if(IsT2)
  {
      fprintf(fpout,"%lld CHN %1x %lld %8.0lf\n", RecNum, Channel, TimeTag, (TimeTag * GlobRes * 1e12));
  }
  else
  {
	  while ((TimeTag * GlobRes * 1e9)/1000000000 >= bin_time)
	  {
		fprintf(fpout,"%g  %12.12lf \n", bin_time-binL,  tmp_counts);
		bin_time += binL;
		tmp_counts = 0;
	}
	tmp_counts ++;
  ...
  switch (RecordType)
  {
   ...
   case rtTimeHarp260NT2:
    ...
    case rtTimeHarp260PT3:
      fprintf(fpout, "TimeHarp260(TimeTag * GlobRes * 1e9)P T3 data\n");
      fprintf(fpout,"\nrecord# chan   nsync truetime/ns dtime\n");
      break;
